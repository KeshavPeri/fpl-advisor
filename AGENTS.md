...
dispatches the subagents in `.Codex/agents/`, moves issues between `status:` labels, appends
...
All work happens on branches named `Codex/ticket-<number>-<slug>`, e.g.
`Codex/ticket-14-gameweek-deadline-countdown`. This matches Codex Routines' default
push restriction — `Codex/`-prefixed branches are always accepted. Never commit to `main`.

## The board is issue labels

**There is no project board in this pipeline.** A Codex Routine runs behind a GitHub
...
**Impeccable and emil-design-eng (installed under `.Codex/skills/`) are invoked exclusively
...
`.Codex/agents/analyst.md` (classifies decisions against the tiers, answers from the brief,
