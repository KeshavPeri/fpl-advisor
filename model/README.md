...
#   opp_team_code, gf, ga
#
# odds columns (fpl_odds output, run 2 — not produced by this ticket): season, gw, kickoff_date,
...
Run 1 accepts and ignores `odds`. It uses from `snapshot`/`snapshots` only price and
...
The gate (docs/model-diagnosis-2026-09-24.md §7):

- **Liveness** — every retrain fold has test rows and non-constant predictions. Hard fail if not.
...
- **Informational, never gated** — 1-GW featured Spearman; captain/top-11 average points from a
  most-owned pool each GW; mean bias on active rows.
