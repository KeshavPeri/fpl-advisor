// Unit tests for scripts/lib/paginate.ts — ticket #43. No Supabase, no
// network: fetchAllPages is exercised against an in-memory array sliced the
...
import { assertRowCountMatches, DEFAULT_PAGE_SIZE, fetchAllPages, RowCountMismatchError, type PageResponse } from './paginate.js'

/** Builds a fetchPage function backed by an in-memory array, the same shape a real `.range(from, to)` Supabase call resolves to. */
function pagedSource<T>(source: readonly T[]): (from: number, to: number) => Promise<PageResponse<T>> {
  return (from, to) => Promise.resolve({ data: source.slice(from, to + 1), error: null })
}
...
describe('fetchAllPages', () => {
  ...
  it('stops immediately and surfaces a page-level error without accumulating further pages', async () => {
    ...
    const fetchPage = (_from: number, _to: number) => {
      callCount++
      return Promise.resolve({ data: null, error: { code: 'PGRST205', message: 'relation "widgets" does not exist' } })
    }
