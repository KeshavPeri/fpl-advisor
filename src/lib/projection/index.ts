/**
 * Projection module — pure computation, no I/O.
 * See product-brief.md §6d for the inputs this module implements.
 */
export * from './types.ts'
export * from './defconRate.ts'
export * from './minutes.ts'
export * from './rates.ts'
export * from './fixture.ts'
export * from './pointValues.ts'
export * from './teamStrength.ts'
export * from './marketOdds.ts'
export * from './expectedPoints.ts'
export * from './bonus.ts'
